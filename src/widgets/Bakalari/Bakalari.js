        ...
        const lessons = day.Atoms.map(lesson => {
          ...
          if (changed) {
            ...
            switch (change.ChangeType) {
              case 'Removed':
              case 'Canceled':
                return { changed, changeType: change.ChangeType, changeDescription: change.Description };
              default:
                break;
            }
          }
          const subject = data.Subjects.find(e => e.Id === lesson.SubjectId).Abbrev;
          const time = parseInt(data.Hours.find(e => e.Id === lesson.HourId).Caption);
              ...
              (day, i) => {
                return (
                  <tr key={i} className='day'>
                    ...
                    {day.lessons.map(
                      (lesson, i) =>
                        lesson ? (<td key={i} className='lesson'>{lesson.subject}</td>) : (<td className='lesson' />)
                    )}
                  </tr>
                );
              }
